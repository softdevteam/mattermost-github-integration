...
> %s""" % (self.user_link(), self.number, self.title,
    ...
    def assigned(self):
        ...
        msg = """%s assigned %s to pull request [#%s %s](%s).""" % (
            self.user_link(),
...
class Wiki(Payload):
    ...
    def updated(self):
        ...
        msg = []
        msg.append(
            "%s changes %s pages in Wiki at %s:".format(
                self.user_link(), len(pages), self.repo_link()
            )
        )
        for page in pages:
